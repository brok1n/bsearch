...
QString CCrashStack::GetExceptionInfo()
{
        ...
        if (m_pException)
        {
                ...
                sprintf(buffer, "Exception Addr:  %08X  ", (long long)E.ExceptionAddress);
                sRet.append(buffer);
...
//                sprintf(buffer, "\nEAX: %08X  EBX: %08X  ECX: %08X  EDX: %08X",  (unsigned int)C.Eax,(unsigned int) C.Ebx, (unsigned int)C.Ecx, (unsigned int)C.Edx);
                sprintf(buffer, "\nEAX: %08X  EBX: %08X  ECX: %08X  EDX: %08X",  (unsigned int)C.Rax,(unsigned int) C.Rbx, (unsigned int)C.Rcx, (unsigned int)C.Rdx);
                sRet.append(buffer);

                memset(buffer, 0, sizeof(buffer));
//                sprintf(buffer, "\nESI: %08X  EDI: %08X  ESP: %08X  EBP: %08X", (unsigned int)C.Esi, (unsigned int)C.Edi, (unsigned int)C.Esp, (unsigned int)C.Ebp);
                sprintf(buffer, "\nESI: %08X  EDI: %08X  ESP: %08X  EBP: %08X", (unsigned int)C.Rsi, (unsigned int)C.Rdi, (unsigned int)C.Rsp, (unsigned int)C.Rbp);
                sRet.append(buffer);

                memset(buffer, 0, sizeof(buffer));
//                sprintf(buffer, "\nEIP: %08X  EFlags: %08X", (unsigned int)C.Eip,(unsigned int) C.EFlags);
                sprintf(buffer, "\nEIP: %08X  EFlags: %08X", (unsigned int)C.Rip,(unsigned int) C.EFlags);
                sRet.append(buffer);
